defmodule Rumbl.VideoControllerTest do
  ...
  @valid_attr %{url: "http://youtu.be", title: "vid", description: "a vid"}
  @invalid_attr %{title: "invalid"}

  defp video_count(query), do: Repo.one(from v in query, select: count(v.id))

  @tag login_as: "max"
  test "creates user video and redirects", %{conn: conn, user: user} do
    conn = post(conn, video_path(conn, :create), video: @valid_attr)
    assert redirected_to(conn) == video_path(conn, :index)
    assert Repo.get_by!(Video, @valid_attr).user_id == user.id
  end

  @tag login_as: "max"
  test "does not create video and renders errors when invalid", %{conn: conn} do
    count_before = video_count(Video)
    conn = post conn, video_path(conn, :create), video: @invalid_attr
  ...
  test "authorizes actions against access by other users", %{conn: conn, user: owner} do
    video = insert_video(owner, @valid_attr)
    ...
    assert_error_sent :not_found, fn ->
      get(conn, video_path(conn, :update, video, video: @valid_attr))
    end
